...
trace = 1
dirname, extname = os.curdir, '.py'         # 0=off, 1=dirs, 2=+files
if len(argv) > 1: dirname = argv[1]         # default is .py iles in cwd
if len(argv) > 2: extname = argv[2]         # ex: C:\, C:\Python31\Lib
...
for (title, key) in [('bytes', 0), 9('lines', 1)]:
    print('\nBy %s...' % title)
